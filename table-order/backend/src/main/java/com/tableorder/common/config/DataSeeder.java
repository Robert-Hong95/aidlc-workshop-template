...
public class DataSeeder implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        // 추천메뉴
        menuRepository.save(new Menu(sid, cat1.getId(), "시그니처 스테이크", 32000, "부드러운 안심 스테이크와 특제 소스", null, 1));
        menuRepository.save(new Menu(sid, cat1.getId(), "트러플 파스타", 18000, "블랙 트러플 오일을 곁들인 크림 파스타", null, 2));
        menuRepository.save(new Menu(sid, cat1.getId(), "해산물 플래터", 45000, "랍스터, 새우, 조개 모듬", null, 3));

        // 메인요리
        menuRepository.save(new Menu(sid, cat2.getId(), "치킨 스테이크", 22000, "허브 마리네이드 치킨 스테이크", null, 1));
        menuRepository.save(new Menu(sid, cat2.getId(), "연어 스테이크", 28000, "노르웨이산 연어 그릴", null, 2));
        menuRepository.save(new Menu(sid, cat2.getId(), "함박 스테이크", 16000, "수제 함박 스테이크와 데미글라스 소스", null, 3));
        menuRepository.save(new Menu(sid, cat2.getId(), "봉골레 파스타", 15000, "바지락 봉골레 파스타", null, 4));

        // 사이드
        menuRepository.save(new Menu(sid, cat3.getId(), "시저 샐러드", 12000, "로메인 상추와 파마산 치즈", null, 1));
        menuRepository.save(new Menu(sid, cat3.getId(), "감자튀김", 8000, "바삭한 감자튀김과 트러플 소금", null, 2));
        menuRepository.save(new Menu(sid, cat3.getId(), "마늘빵", 6000, "갈릭 버터 바게트", null, 3));
        menuRepository.save(new Menu(sid, cat3.getId(), "콘 수프", 7000, "부드러운 크림 콘 수프", null, 4));

        // 음료
        menuRepository.save(new Menu(sid, cat4.getId(), "아메리카노", 5000, "에스프레소 더블샷", null, 1));
        menuRepository.save(new Menu(sid, cat4.getId(), "카페라떼", 6000, "부드러운 우유와 에스프레소", null, 2));
        menuRepository.save(new Menu(sid, cat4.getId(), "레몬에이드", 7000, "생레몬 에이드", null, 3));
        menuRepository.save(new Menu(sid, cat4.getId(), "콜라", 3000, "코카콜라 355ml", null, 4));
        menuRepository.save(new Menu(sid, cat4.getId(), "맥주", 8000, "생맥주 500ml", null, 5));
    }
}
