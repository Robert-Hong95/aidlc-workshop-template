...
export function TableCardView({ tables, onComplete, onHistory }: {
  ...
  return (
    <div data-testid="table-card-view" className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {tables.map((t) => (
        <div key={t.id} className={`border rounded-lg p-4 ${t.hasActiveSession ? 'border-green-300 bg-green-50' : 'border-gray-200'}`}>
          <div className="flex justify-between items-center mb-2">
            <span className="font-semibold text-[#232F3E]">테이블 {t.tableNo}</span>
            <span className={`text-xs px-2 py-0.5 rounded-full ${t.hasActiveSession ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{t.hasActiveSession ? '활성' : '비활성'}</span>
          </div>
          <div className="flex gap-2">
            {t.hasActiveSession && <Button size="sm" onClick={() => onComplete(t)}>완료</Button>}
